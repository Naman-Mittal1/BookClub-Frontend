...
const LoginModal = ({ isOpen, onRequestClose, onRegisterClick }) => {
  ...
  return (
    <>
      {isOpen && (
        <div className=" fixed inset-0 bg-opacity-25 backdrop-blur"></div>
      )}
      <div
        className={`${
          isOpen ? "fixed" : "hidden"
      ...
      >
        <div className="bg-gray-900 p-8 rounded-lg relative lg:w-2/4 xl:1/3 max-w-lg">
          ...
          <h2 className="text-white text-xl  font-semibold mb-2 text-center pb-3">
            Login
          </h2>
    ...
    >
      {passwordVisible ? <FaEyeSlash /> : <FaEye />}
          ...
          <button className="flex gap-2 justify-center bg-transparent border-2 border-white text-white px-4 py-2 rounded hover:bg-gray-800 w-full mb-4">
            <svg
              className="h-5 align-middle w-6"
